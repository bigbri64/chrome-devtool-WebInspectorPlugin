...
//            ];
        ...
        if (WebInspectorPlugin._OPTIONS_['save-curl-command-list']) {
            var _curl_command_delegate_ = function (data) {

                var _fix_filePath = data.filePath;
                _fix_filePath = _fix_filePath.replace(/\\/g,'/');
                _fix_filePath = _fix_filePath.replace(/^\/?\.\//g,'');

                var _TEMPLATE_ = 'curl --create-dirs {FILE_URL} -o ./{FILE_PATH}/{FILE_NAME}';
                return _TEMPLATE_.replace(/{FILE_URL}/g, data.contentURL)
                          .replace(/{FILE_PATH}/g, _fix_filePath)
                          .replace(/{FILE_NAME}/g, data.fileName);
            };
            var _curl_cmd_list = getValuesByPropertyName(sourceFileList, _curl_command_delegate_, filterInvalidURLs);

            var _curl_cmd_ = _curl_cmd_list.join('\n');
            _curl_cmd_ = _curl_cmd_ + '\n';
            saveAsCSV(_curl_cmd_, 'curl-command-list');
        }
