      ...
      try {
        ...
        if (!inspectedPageDomain) {
          ...
          try {
            ...
            var _sourceCodeOriginList = _projects.map(function (_project) {
              return _project._uiSourceCodesList.map(function (_uiSourceCode) {
                return _uiSourceCode._origin;
              });
            });
            ...
            _sourceCodeOriginList = _flatten(_sourceCodeOriginList);

            console.log('_sourceCodeOriginList = ' + JSON.stringify(_sourceCodeOriginList));

            inspectedPageDomain = _sourceCodeOriginList.slice(0, 1);
            // inspectedPageDomain = _sourceCodeOriginList.slice(-1);
          }
          catch (error) {
            console.error(error);
            inspectedPageDomain = 'page-domain-undefined';
          }
...
//                        _reference.requestContent(_callback);
    ...
    var saveAsCSV = function (dataList, suffix) {
      ...
      prefix = (typeof prefix === 'string') ? prefix : String(prefix);
